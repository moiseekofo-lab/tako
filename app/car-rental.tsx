...
const CAR_RENTAL_PT: Record<string, string> = {
  ...
  'Luxe': 'Luxo', 'à partir de': 'a partir de', 'ou similaire': 'ou similar', 'Options et services inclus': 'Opções e serviços incluídos',
...
const extras = [
  ...
  { key: 'driver', title: 'Avec chauffeur', subtitle: 'Inclus dans toutes les locations', icon: 'account-outline', price: 0 },
...
export default function CarRental() {
  ...
  const [hasLiveVehicles,setHasLiveVehicles]=useState(false);
  const [vehicleKey, setVehicleKey] = useState('economy');
  const [selectedExtras, setSelectedExtras] = useState<string[]>(['driver']);
  ...
  const vehicle = vehicles.find((item) => item.key === vehicleKey) ?? vehicles[0] ?? defaultVehicles[0];
  useEffect(()=>{getRentalVehicles().then(result=>{const remote:AppRentalVehicle[]=(result?.vehicles||[]).map((item:any)=>({key:item.id,label:item.category,model:`${item.brand} ${item.model}`,details:`${item.seats} places • ${item.luggage} bagages`,image:item.imageUrl?{uri:item.imageUrl}:categoryImages[item.category]||categoryImages['Économique'],price:Number(item.dailyPrice)}));if(remote.length){setVehicles(remote);setVehicleKey(remote[0].key);setHasLiveVehicles(true)}else{setVehicles([...defaultVehicles]);setHasLiveVehicles(false)}}).catch(()=>{setVehicles([...defaultVehicles]);setHasLiveVehicles(false)})},[]);
  ...
  const toggle = (key: string) => { if(key==='driver') return; setSelectedExtras((list) => list.includes(key) ? list.filter((item) => item !== key) : [...list, key]); };
  ...
  return <View style={styles.screen}>
    <View style={[styles.page, { paddingTop: insets.top + 8, paddingBottom: insets.bottom + 12 }]}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollBody} showsVerticalScrollIndicator={false}>
        ...
        {step === 2 && <>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.vehicleList}>
            {vehicles.map((item) => { const active = hasLiveVehicles&&item.key === vehicleKey; return <TouchableOpacity disabled={!hasLiveVehicles} key={item.key} style={[styles.vehicle,!hasLiveVehicles&&styles.vehicleDisabled,active && styles.vehicleActive]} onPress={() => setVehicleKey(item.key)}>
        ...
        {step === 3 && <>
          ...
          <View style={styles.summary}>
            ...
            <Summary icon="time-outline" label="Durée" value={durationText} /><Summary icon="car-outline" label="Véhicule" value={`${vehicle.label} • ${vehicle.model} ou similaire`} />
      ...
      <TouchableOpacity disabled={step===2&&!hasLiveVehicles} style={[styles.button,step===2&&!hasLiveVehicles&&{opacity:.45}]} onPress={() => {
...
const styles = StyleSheet.create({
  ...
  vehicleList: { gap: 10, paddingBottom: 15 }, vehicle: { width: 158, minHeight: 226, borderWidth: 1, borderColor: '#DDE1EA', borderRadius: 12, padding: 10, alignItems: 'center', justifyContent: 'center' }, vehicleDisabled:{opacity:.42,backgroundColor:'#ECEFF3',borderColor:'#C8CDD5'}, vehicleActive: { borderWidth: 2, borderColor: BLUE, backgroundColor: '#F7FAFF' }, checkBadge: { position: 'absolute', zIndex: 2, top: 7, right: 7, width: 26, height: 26, borderRadius: 13, backgroundColor: BLUE, alignItems: 'center', justifyContent: 'center' }, vehicleImage: { width: 138, height: 76 }, vehicleName: { color: NAVY, fontSize: 16, fontWeight: '900', marginTop: 3 }, vehicleModel: { color: NAVY, fontSize: 14, fontWeight: '800', marginTop: 3, textAlign: 'center' }, similar: { color: '#687184', fontSize: 12, fontWeight: '600' }, vehicleDetails: { color: '#687184', fontSize: 14, marginTop: 3, textAlign: 'center' }, muted: { color: '#737A89', fontSize: 14, marginTop: 3 }, priceSmall: { color: BLUE, fontSize: 14, fontWeight: '900', marginTop: 3 }, unavailableNotice:{flexDirection:'row',alignItems:'center',gap:8,backgroundColor:'#F2F4F7',borderRadius:9,padding:11,marginBottom:12},unavailableText:{flex:1,color:'#667085',fontSize:12,lineHeight:17,fontWeight:'600'},
  optionCard: { borderWidth: 1, borderColor: '#DDE1EA', borderRadius: 12, overflow: 'hidden' }, option: { minHeight: 72, paddingHorizontal: 13, flexDirection: 'row', alignItems: 'center', gap: 12, borderBottomWidth: 1, borderBottomColor: '#E7EAF0' }, optionIncluded: { backgroundColor: '#F3F8FF' }, noVehicles:{minHeight:150,borderWidth:1,borderColor:'#DDE1EA',borderRadius:12,backgroundColor:'#F7FAFF',alignItems:'center',justifyContent:'center',padding:18}, iconBox: { width: 40, height: 40, borderRadius: 9, backgroundColor: '#F0F6FF', alignItems: 'center', justifyContent: 'center' }, optionTitle: { color: NAVY, fontSize: 16, fontWeight: '800' }, includedBadge: { color: BLUE, backgroundColor: '#E5F1FF', borderRadius: 10, paddingHorizontal: 7, paddingVertical: 4, fontSize: 10, fontWeight: '900', overflow: 'hidden' }, checkbox: { width: 24, height: 24, borderRadius: 5, borderWidth: 1.5, borderColor: '#A8ADB7', alignItems: 'center', justifyContent: 'center' }, checkboxActive: { borderColor: BLUE, backgroundColor: BLUE },
