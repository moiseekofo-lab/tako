...
export default function CarRental() {
  ...
  const [vehicles,setVehicles]=useState<AppRentalVehicle[]>([]);
    ...
    <View style={[styles.page, { paddingTop: insets.top + 8, paddingBottom: insets.bottom + 12 }]}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollBody} showsVerticalScrollIndicator={false}>
        ...
        {step === 2 && <>
          ...
          {vehicles.length===0&&<View style={styles.noVehicles}><Ionicons name="car-outline" size={34} color={BLUE}/><Text style={styles.optionTitle}>Aucun véhicule disponible actuellement</Text><Text style={styles.muted}>Les véhicules ajoutés par l’administration apparaîtront ici.</Text></View>}
      ...
      <TouchableOpacity disabled={step===2&&vehicles.length===0} style={[styles.button,step===2&&vehicles.length===0&&{opacity:.45}]} onPress={() => {
