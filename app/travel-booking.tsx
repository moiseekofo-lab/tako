...
export default function TravelBooking() {
  ...
  return <View style={styles.page}><ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false} contentContainerStyle={[styles.scroll, { paddingBottom: 106 + insets.bottom }]}>
    ...
    <View style={styles.body}>
      ...
      {passengers.map((passenger, index) => <View style={styles.passengerCard} key={index}>
        ...
        <View style={styles.inputGrid}>
          <Input fullWidth={narrow} label="Nom complet" icon="person-outline" value={passenger.name} onChangeText={(v: string) => updatePassenger(index, 'name', v)} />
          <Input fullWidth={narrow} label="Téléphone" icon="call-outline" value={passenger.phone} keyboardType="phone-pad" onChangeText={(v: string) => updatePassenger(index, 'phone', v)} />
          <Input fullWidth={narrow} label="Pièce d’identité (optionnel)" icon="card-outline" value={passenger.identity} placeholder="Sélectionner" onChangeText={(v: string) => updatePassenger(index, 'identity', v)} />
          <Input fullWidth={narrow} label="Email (optionnel)" icon="mail-outline" value={passenger.email} keyboardType="email-address" onChangeText={(v: string) => updatePassenger(index, 'email', v)} />
        </View>
      </View>)}
...
function Input({ fullWidth, ...props }: any) { return <View style={[styles.inputWrap, fullWidth && styles.inputWrapFull]}><Text style={styles.inputLabel}>{props.label}</Text><View style={styles.input}><Ionicons name={props.icon} size={17} color="#071F67" /><TextInput {...props} style={styles.textInput} placeholderTextColor="#9A9DA5" /></View></View>; }
...
const styles = StyleSheet.create({
  page:{flex:1,backgroundColor:'#F8FAFE'},scrollView:{flex:1},scroll:{paddingBottom:106},hero:{minHeight:104,paddingHorizontal:16,paddingBottom:8,flexDirection:'row',alignItems:'center'},back:{width:42,height:42,alignItems:'center',justifyContent:'center'},heroCopy:{flex:1,alignItems:'center'},heroTitle:{color:'#061F68',fontSize:21,fontWeight:'900'},heroSubtitle:{color:'#667085',fontSize:13,marginTop:2},heroSide:{width:42},body:{backgroundColor:'#F8FAFE',paddingHorizontal:16,paddingTop:2,paddingBottom:12},
  ...
  sectionTitle:{flexDirection:'row',alignItems:'center',gap:8,marginTop:14,marginBottom:8},sectionTitleText:{fontSize:17,fontWeight:'900',color:'#061F68'},passengerCard:{backgroundColor:'#fff',borderRadius:12,padding:12,marginBottom:6,borderWidth:1,borderColor:'#ECEEF3'},passengerTitle:{fontSize:13,fontWeight:'900',marginBottom:8},inputGrid:{flexDirection:'row',flexWrap:'wrap',columnGap:10,rowGap:9},inputWrap:{width:'48.5%'},inputWrapFull:{width:'100%'},inputLabel:{fontSize:10,color:'#555B66',marginBottom:4},input:{height:40,borderWidth:1,borderColor:'#D9DDE5',borderRadius:7,paddingHorizontal:9,flexDirection:'row',alignItems:'center',gap:7},textInput:{flex:1,fontSize:11,color:'#111',paddingVertical:0,minWidth:0},
  seatCard:{backgroundColor:'#fff',borderRadius:12,padding:11,borderWidth:1,borderColor:'#ECEEF3'},tabs:{height:40,borderWidth:1,borderColor:'#E5E8EF',borderRadius:7,flexDirection:'row',overflow:'hidden'},tab:{flexDirection:'row',gap:5,alignItems:'center',justifyContent:'center',paddingHorizontal:5},tabPlan:{width:'44%'},tabAutomatic:{width:'56%'},tabActive:{backgroundColor:'#EAF2FF'},tabText:{fontSize:11,color:'#0758D0',fontWeight:'800'},tabMuted:{fontSize:10,color:'#555B66'},seatContent:{flexDirection:'row',paddingTop:11,alignItems:'flex-start'},seatContentNarrow:{flexDirection:'column'},busPlan:{width:'60%',backgroundColor:'#F2F6FC',borderRadius:29,padding:9},busPlanNarrow:{width:'100%'},windshield:{height:20,borderRadius:18,backgroundColor:'#CBD6E9',marginBottom:7},columnLabels:{flexDirection:'row',alignItems:'center',marginBottom:5},rowNumberSpace:{width:21},columnLabel:{flex:1,textAlign:'center',fontSize:9,color:'#394150'},seatViewport:{height:166},seatViewportCompact:{height:148},seatRows:{gap:5,paddingBottom:4},seatRow:{flexDirection:'row',alignItems:'center',gap:5},rowNumber:{width:16,textAlign:'center',fontSize:8,color:'#394150'},seat:{flex:1,height:27,borderRadius:4,backgroundColor:'#128BF1',alignItems:'center',justifyContent:'center'},seatUnavailable:{backgroundColor:'#D8DADF'},seatSelected:{backgroundColor:'#08A56E'},legend:{flex:1,paddingLeft:12,paddingTop:10},legendRow:{flexDirection:'row',alignItems:'center',gap:7,marginBottom:9},legendColor:{width:13,height:13,borderRadius:3},legendText:{fontSize:9,color:'#181A1F'},selectedBox:{marginTop:10,borderWidth:1,borderColor:'#E2E5EB',borderRadius:8,padding:9},selectedLabel:{fontSize:9,color:'#333'},selectedValue:{fontSize:16,fontWeight:'900',color:'#08A56E',marginTop:4},
  bottomBar:{position:'absolute',left:0,right:0,bottom:0,backgroundColor:'#F8FAFE',paddingHorizontal:16,paddingTop:6,borderTopWidth:1,borderTopColor:'#E8EBF1',shadowColor:'#10275C',shadowOpacity:.1,shadowRadius:8,elevation:10},protected:{flexDirection:'row',alignItems:'center',gap:5,marginLeft:4,marginBottom:5},protectedText:{fontSize:9,color:'#667080'},payButton:{height:52,borderRadius:10,backgroundColor:'#072B84',flexDirection:'row',alignItems:'center',justifyContent:'space-between',paddingHorizontal:21},payDisabled:{opacity:.7},payText:{color:'#fff',fontSize:15,fontWeight:'900'},
});
