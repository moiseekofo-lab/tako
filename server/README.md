...
## OTP Email, WhatsApp puis SMS avec Infobip
...
- `INFOBIP_EMAIL_FROM` : adresse expéditrice dont le domaine est vérifié dans Infobip
- `INFOBIP_EMAIL_FROM_NAME` : nom affiché de l’expéditeur, par exemple `TaKo`
...
Les adresses email utilisent l’API Email Infobip. La clé API doit posséder le
scope `email:message:send` (ou `email:manage`) et le domaine de
`INFOBIP_EMAIL_FROM` doit être vérifié et actif dans le compte Infobip. Le code
OTP n’est jamais renvoyé à l’application.
